...
Главный класс — `SchedulerJobRunner` (в 3.x — `airflow-core/src/airflow/jobs/scheduler_job_runner.py`).
...
def _create_dagruns_for_dags(self, session):
    ...
    for dag in dags:
        dag_run = create_dagrun(
            ...
            dag_version_id=dag.current_version_id,  # 3.x — pin to version
        )
        session.add(dag_run)
...
- `dag_version_id` (3.x) — фиксируем версию bundle на момент создания DagRun.
- Asset-triggered runs — из таблицы `dataset_dag_run_queue` (если все необходимые datasets обновлены).
...
def _schedule_dag_run(self, dag_run, session):
    dag = serialized_dag_to_dag(dag_run.dag_version_id)
...
| `dag_dir_list_interval` | 300 | Scan папки DAGs на новые файлы (relevant for Local Bundle) |
