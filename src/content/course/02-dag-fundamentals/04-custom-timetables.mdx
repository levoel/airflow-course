...
<DiagramContainer title="Timetable interaction с scheduler" color="indigo">
  <FlowColumn>
    <FlowRow>
      <DataBox label="Scheduler main loop" tooltip="SchedulerJob каждый tick (default 5s) проходит по active DAGs и для каждого решает — нужно ли создать новый DagRun. Для этого вызывает timetable.next_dagrun_info().">tick every scheduler_heartbeat_sec</DataBox>
    </FlowRow>
    <Arrow direction="down" label="next_dagrun_info(last_interval, restriction)" />
    <FlowRow>
      <DataBox label="Timetable.next_dagrun_info()" tooltip="Главный метод. Принимает last_automated_data_interval (когда был последний run) и TimeRestriction (start_date, end_date, catchup). Возвращает DagRunInfo или None если runs больше не нужны.">compute next run</DataBox>
    </FlowRow>
    <Arrow direction="down" />
    <FlowRow>
      <DataBox label="DagRunInfo" tooltip="Структура с двумя полями: run_after (когда scheduler должен создать DagRun) и data_interval (DataInterval start, end — какие данные этот run обрабатывает). Scheduler ждёт до run_after момента.">run_after: DateTime</DataBox>
      <DataBox label="DataInterval(start, end)" tooltip="Временное окно которое представляет этот DagRun. Для @daily: start=date 00:00, end=date+1 00:00. Доступно в task через context['data_interval_start'] и data_interval_end.">обрабатываемый период</DataBox>
    </FlowRow>
    <Arrow direction="down" label="когда now() >= run_after" />
    <FlowRow>
      <DataBox label="Scheduler creates DagRun" tooltip="INSERT в dag_run table: dag_id, run_id, data_interval_start/end, state='running'. Затем создаются TaskInstance rows для всех tasks DAG. Tasks становятся eligible для scheduling.">dag_run row created, TIs spawned</DataBox>
    </FlowRow>
  </FlowColumn>
</DiagramContainer>
